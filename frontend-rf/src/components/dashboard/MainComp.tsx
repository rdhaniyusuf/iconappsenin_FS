    ...
    return (
        <div className="gap-2 grid grid-cols-2 sm:grid-cols-4">
            {topCardList.map((item, index) => (
                <Card key={index} className="rounded" isPressable shadow="sm" onPress={() => console.log("item pressed")}>
                    <CardBody className="overflow-visible p-0 rounded">
                        {/* need add information count of object */}
                    </CardBody>
                    <CardFooter className="text-small justify-between">
                        <b>{item.title}</b>
                        <Chip className="rounded items-center" color={item.color as "danger" | "warning" | "default" | "success" | "primary" | "secondary"} startContent={<item.icon size={16} />} variant="flat">
                            {item.count}
                        </Chip>
                    </CardFooter>
                </Card>
...
const INITIAL_VISIBLE_COLUMNS = ["name", "role", "clockIn", "status", "actions"];
...
const BottomTable = () => {
    ...
    const headerColumns = React.useMemo(() => {
        ...
        return tableColumns.filter((tableColumns) => Array.from(visibleColumns).includes(tableColumns.uid));
    }, [visibleColumns]);
...
export { TopCardComp, BottomTable, BottomActivity };